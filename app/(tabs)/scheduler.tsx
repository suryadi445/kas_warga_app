...
export default function SchedulerScreen() {
    ...
    async function openEdit(s: Schedule) {
        ...
        const urls = await ensureDownloadUrls((s as any).images || []);
        setImages(urls);
        setModalVisible(true);
    }
    ...
    async function save() {
        ...
        try {
            if (editingId) {
                ...
                await updateDoc(ref, { activityName, date, time, frequency, location, description, days: daysToSave, images: uploadedUrls, updatedAt: serverTimestamp() });
                setSelectedDays(daysToSave);
                showToast(t('schedule_updated', { defaultValue: 'Schedule updated' }), 'success');
            } else {
                const daysToSave = sortDaysMondayFirst(selectedDays);
                const docRef = await addDoc(collection(db, 'schedules'), { activityName, date, time, frequency, location, description, days: daysToSave, createdAt: serverTimestamp() });
                if (images?.length) {
                    const urls = await uploadAllImagesAndReturnUrls(images, docRef.id);
                    await updateDoc(doc(db, 'schedules', docRef.id), { images: urls });
                }
                setSelectedDays(daysToSave);
    ...
    async function removeConfirmed() {
        ...
        try {
            ...
            try {
                const refDoc = doc(db, 'schedules', itemToDelete);
                ...
                console.warn('Failed deleting schedule images', errImgs);
            }
    ...
    // Image helpers
    async function uploadImageToStorage(uri: string, scheduleId: string): Promise<string> {
        if (!uri) return '';
        if (uri.startsWith('http')) return uri;
        try {
            const response = await fetch(uri);
            const blob = await response.blob();
            const filename = `schedules/${scheduleId}_${Date.now()}_${Math.random().toString(36).slice(2)}.jpg`;
    ...
    async function uploadAllImagesAndReturnUrls(uris: string[], scheduleId: string) {
        const urls: string[] = [];
        for (let i = 0; i < uris.length; i++) {
            const u = uris[i];
            if (!u) continue;
            if (u.startsWith('http')) { urls.push(u); continue; }
            const url = await uploadImageToStorage(u, scheduleId);
            urls.push(url);
        }
        return urls;
    }

    // Use shared storage helper deleteImageFromStorageByUrl
    ...
    async function ensureDownloadUrls(uris: string[]) {
        ...
        const converted = await Promise.all(uris.map(async (u) => {
            try {
                ...
                let path = u;
                if (u.startsWith('gs://')) { path = u.replace(/^[^/]+:\/\/[\w.-]+\//, ''); }
                const m = (u || '').match(/\/o\/([^?]+)/);
    ...
    async function pickImages() {
        ...
        try {
            ...
            const res = await ImagePicker.launchImageLibraryAsync({ ...(MEDIA_IMAGES ? { mediaTypes: MEDIA_IMAGES } : {}), allowsMultipleSelection: true, quality: 0.8 });
            if (!res.canceled && res.assets) {
                const uris = res.assets.map((a: any) => a.uri);
                await new Promise(resolve => setTimeout(resolve, 250));
                setImages(prev => [...prev, ...uris]);
            }
        } catch (err) { console.warn('Image picker error', err); } finally { setImagesLoading(false); }
    }
    ...
    async function handleReplaceImageInEdit(index: number) {
        try {
            ...
            await updateDoc(doc(db, 'schedules', editingId), { images: prevImages });
            if (oldUrl && (oldUrl.startsWith('http://') || oldUrl.startsWith('https://') || oldUrl.startsWith('gs://'))) {
                ...
                if (!deleted) {
                    console.warn('Failed to delete old schedule image', oldUrl);
    ...
    async function handleRemoveImageInEdit(index: number) {
        try {
            ...
            if (editingId) {
                setOperationLoading(true);
                await updateDoc(doc(db, 'schedules', editingId), { images: current });
                if (removed && (removed.startsWith('http://') || removed.startsWith('https://') || removed.startsWith('gs://'))) {
                    ...
                    if (!deleted) {
                        console.warn('Failed to delete removed schedule image', removed);
                        showToast?.(t('failed_to_delete_storage_image', { defaultValue: 'Failed to delete previous storage image. Check storage rules.' }), 'error');
                    }
                }
            }
                                        ...
                                        <View style={{
                                            ...
                                            {item.images && item.images.length > 0 && (
                                                <TouchableOpacity onPress={() => showImagePreview(item.images || [], 0)} style={{ marginTop: 6 }}>
                                                    <Text style={{ color: '#3B82F6', fontWeight: '700' }}>{t('image_count_label', { count: item.images.length, defaultValue: `image : ${item.images.length} (totalnya)` })}</Text>
                                                </TouchableOpacity>
                                            )}
                                        </View>
                                    </View>
